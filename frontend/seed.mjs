...
async function seed() {
  ...
  const formattedProducts = allProducts.map(p => ({
    name: p.title,
    description: `A beautifully curated ${p.category} gift perfect for any occasion.`,
    price: p.price,
    compare_at_price: p.originalPrice || null,
    stock: Math.floor(Math.random() * 50) + 10,
    intent: p.category,
    features: ["Premium quality", "Beautifully packaged"],
    images: [p.image]
  }));
  ...
  if (error) {
    ...
    console.log("Successfully seeded 30 products into Supabase!");
  }
}
